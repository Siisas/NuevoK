...
#Region "Page"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            lblmsg.Text = Nothing
            Sesion_Local = Obj_Session.CrearValidar_SesionLocal("Control_Escalamiento_SD", ViewState("Session"))
...
#Region "Métoddos"
    ...
    Private Sub Consultar_Asignados()
        ViewState("Dtg_Estad") = Objdatos.Cons_Asignacion_Control_SD
...
#Region "Btn's"
    Protected Sub Btn_Buscar_Click(sender As Object, e As EventArgs) Handles Btn_Buscar.Click
        Try
            ...
            End If
            If Drl_EstadoSD.SelectedIndex > 0 Then
                Filtro.Add("EstadoSD", Drl_EstadoSD.SelectedItem.Text)
            End If
            ...
            End If
            If Txt_FRegBandejaIni.Text <> Nothing Then
                Filtro.Add("FRegBandejaIni", Txt_FRegBandejaIni.Text)
            End If
            If Txt_FRegBandejaFin.Text <> Nothing Then
                Filtro.Add("FRegBandejaFin", Txt_FRegBandejaFin.Text)
            End If
            If Txt_FRegSdIni.Text <> Nothing Then
                Filtro.Add("FRegSdIni", Txt_FRegSdIni.Text)
            End If
            If Txt_FRegSdFin.Text <> Nothing Then
                Filtro.Add("FRegSdFin", Txt_FRegSdFin.Text)
            End If
            If Filtro.Count <= 0 Then
                Throw New Exception("Seleccione algun filtro de consulta!")
            End If
            ViewState("Dtg_General") = Objdatos.Cons_Escalamiento_SD(Filtro)
            Dtg_General.DataSource = ViewState("Dtg_General")
            Dtg_General.DataBind()
            Lbl_Cuenta.Text = ViewState("Dtg_General").Rows.Count
            If ViewState("Dtg_General").Rows.Count <= 0 Then
        ...
        Try
            ...
            For Each Row As GridViewRow In Dtg_General.Rows
                For I As Integer = 0 To Dtg_General.Columns.Count - 1
                    ...
                    End If
                Next
                Objdatos.Asignacion_Control("SD")
                num += 1
            Next
    ...
    Protected Sub Btn_Limpiar_Click(sender As Object, e As EventArgs) Handles Btn_Limpiar.Click
        Try
            ...
            Txt_FRegSdIni.Text = Nothing
            Txt_FRegSdFin.Text = Nothing
        Catch ex As Exception
            Obj_G_D.Val_Ex(ex)
        End Try
    End Sub
